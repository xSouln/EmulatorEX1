...
using static Emulator_ex_0_.EmulatorDivX.POA.Types;
    ...
    public partial class POA
    {
        public unsafe class Responses : xResponseBuilder
        {
            ...
            public unsafe class Get
            {
                ...
                {
                    Header = REQ,
                    Name = "POA." + nameof(EActions.GetState),
                    EventReceive = (response, result) =>
                    {
                        transmite(((xTcpServer.ClientStream)response.Context).Send, RES, new ReportT(), EActions.GetState);
